...
- `NowUIQuickStartOverlay.cs`: built-in render pipeline `OnPostRender` overlay
  with measured layout, a replaying animated text-gradient title, a CSS-style
  gradient paint, a soft capsule mask shared by gradient and text draws, a
  half-resolution composed SDF cutout mask, and a button.
